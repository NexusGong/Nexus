"""
OCR服务模块（使用百度OCR + 豆包API后处理）
负责图片文字识别和聊天内容提取，优化为使用百度OCR快速识别，然后用LLM判断发言人位置
"""
...
class DoubaoOCRService:
    """OCR服务类（使用百度OCR + 豆包API后处理）"""
    
    def __init__(self):
        """初始化OCR服务"""
        # 百度OCR配置（可选，如果没有配置则回退到豆包）
        self.baidu_app_id = settings.baidu_app_id
        self.baidu_api_key = settings.baidu_api_key
        self.baidu_secret_key = settings.baidu_secret_key
        self.use_baidu_ocr = bool(self.baidu_api_key and self.baidu_secret_key)
        self._baidu_access_token = None
        self._baidu_token_expires = 0
        
        # 豆包API配置（用于后处理或作为主要OCR）
    ...
    async def _get_baidu_access_token(self) -> str:
        ...
        # 如果token还有效，直接返回
        if self._baidu_access_token and time.time() < self._baidu_token_expires:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if "access_token" in result:
                self._baidu_access_token = result["access_token"]
                # token有效期通常是30天，但为了安全，我们设置为29天
                self._baidu_token_expires = time.time() + (29 * 24 * 3600)
                return self._baidu_access_token
            else:
                raise Exception(f"获取百度access_token失败: {result}")
    
    async def _baidu_ocr_recognition(self, image_data: bytes) -> Dict[str, Any]:
        """调用百度OCR API进行识别（标准含位置版，支持多语言）"""
        try:
            access_token = await self._get_baidu_access_token()
            # 使用标准含位置版API（返回location信息）
            ...
            data = {
                ...
                "language_type": "CHN_ENG",  # 支持中英文混合识别（CHN_ENG, ENG, JAP, KOR等）
            }
            ...
            logger.error(f"百度OCR调用超时: {e}")
            raise Exception("百度OCR识别超时，请稍后重试")
        except httpx.HTTPError as e:
            logger.error(f"百度OCR HTTP请求失败: {e}")
            raise Exception(f"百度OCR服务请求失败: {str(e)}")
        except Exception as e:
            logger.error(f"百度OCR调用失败: {e}")
            raise
    ...
    async def extract_text_from_images(
        ...
        if mode == "quality":
            ...
            # 默认使用极速模式（百度OCR）
            return await self._extract_with_baidu_ocr(images_data, image_formats)
    ...
    def _process_ocr_blocks(self, blocks: list[dict]) -> tuple[list[dict], str]:
        ...
        # 3. 使用聚类思想找出左右分组的最佳分界点
        center_x_list = [b.get("center_x", 0) for b in blocks]
        center_x_sorted = sorted(center_x_list)
        
        # 计算x坐标的聚类中心（K-means的思想，K=2）
        # 使用图片中心作为初始分界点
        image_center_x = min_x + image_width / 2
        
        # 迭代找到最佳分界点
        x_threshold = image_center_x
        best_score = float('inf')
        
        # 尝试多个候选阈值（图片中心附近的点）
        candidate_thresholds = [image_center_x]
        if len(center_x_sorted) > 2:
            # 添加中位数、三分位点等
            candidate_thresholds.extend([
                center_x_sorted[len(center_x_sorted) // 3],
                center_x_sorted[len(center_x_sorted) // 2],
                center_x_sorted[len(center_x_sorted) * 2 // 3],
            ])
        
        # 计算每个候选阈值的分组质量（使用组内距离小、组间距离大的原则）
        for candidate_threshold in candidate_thresholds:
            left_group = [cx for cx in center_x_list if cx < candidate_threshold]
            right_group = [cx for cx in center_x_list if cx >= candidate_threshold]
            ...
            left_var = sum((x - left_mean) ** 2 for x in left_group) / len(left_group) if left_group else 0
            right_var = sum((x - right_mean) ** 2 for x in right_group) / len(right_group) if right_group else 0
            
            # 计算组间距离（越大越好）
            group_distance = abs(right_mean - left_mean)
            
            # 评分：组内方差小 + 组间距离大 = 好分组
            # 分数越小越好（组内方差小，组间距离大）
            score = (left_var + right_var) / max(group_distance, 1)
            
            if score < best_score:
                best_score = score
                x_threshold = candidate_threshold
        ...
        # 如果分组结果不合理（一侧数量为0），使用备用方案
        if len(left_blocks) == 0 or len(right_blocks) == 0:
            # 使用中位数作为分界点
            if len(center_x_sorted) > 1:
                x_threshold = center_x_sorted[len(center_x_sorted) // 2]
                ...
                logger.warning(f"[OCR][side] 分组异常，使用中位数阈值: x={x_threshold:.1f}, 左侧={len(left_blocks)}个块, 右侧={len(right_blocks)}个块")
    ...
    async def extract_text_from_image(
        ...
        try:
            ...
            # 步骤1: 使用百度OCR快速识别
            t_ocr0 = time.perf_counter()
            ocr_result = await self._baidu_ocr_recognition(image_data)
            t_ocr1 = time.perf_counter()
            logger.info(f"[OCR][baidu] 百度OCR识别耗时: {(t_ocr1 - t_ocr0):.2f}s")
            
            # 步骤2: 提取文字块（与批量识别相同的格式）
            t_process0 = time.perf_counter()
            words_result = ocr_result.get("words_result", [])
            all_blocks = []
            ...
            metadata = {
                "confidence": 0.9,
                "language": ocr_result.get("language", "中文"),
                "structured_messages": messages,
                "participants": participants,
                "ocr_method": "baidu_ocr",
                "word_count": len(all_blocks)
            ...
            logger.info(f"OCR识别成功，文本长度: {len(text_content)}，消息数: {len(messages)}")
